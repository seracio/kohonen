...
var random = function random(size) {
  ...
  return _.map(function (i) {
    return d3Random.randomNormal(mean, deviation)();
  }, _.range(0, size));
};
...
function () {
  ...
  function Kohonen(_ref) {
    ...
    this.scaleStepLearningCoef = d3Scale.scaleLinear().clamp(true).domain([0, maxStep]).range([maxLearningCoef, minLearningCoef]); // decrease neighborhood with time

    this.scaleStepNeighborhood = d3Scale.scaleLinear().clamp(true).domain([0, maxStep]).range([maxNeighborhood, minNeighborhood]); // retrive min and max for each feature

    var unnormalizedExtents = _.flow(_.unzip, _.map(d3Array.extent))(data); // build scales for data normalization


    var scales = unnormalizedExtents.map(function (extent) {
      return d3Scale.scaleLinear().domain(extent).range([0, 1]);
    }); // build normalized data

    this.data = this.normalize(data, scales); // then we store means and deviations for normalized datas

    this.means = _.flow(_.unzip, _.map(d3Array.mean))(this.data);
    this.deviations = _.flow(_.unzip, _.map(d3Array.deviation))(this.data); // On each neuron, generate a random vector v
    // of <size> dimension
  ...
  _createClass(Kohonen, [{
    ...
    value: function umatrix() {
      ...
      return _.map(function (n) {
        return d3Array.mean(findNeighors(n).map(function (nb) {
...
exports.hexagonHelper = hexagon;
exports.vectorHelper = vector;
